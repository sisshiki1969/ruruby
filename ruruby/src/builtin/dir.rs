...
fn home(_: &mut VM, _self_val: Value, args: &Args2) -> VMResult {
    args.check_args_num(0)?;
    let home_dir = dirs::home_dir().unwrap_or(PathBuf::new());
    Ok(Value::string(conv_pathbuf(&home_dir)))
}

fn pwd(_: &mut VM, _self_val: Value, args: &Args2) -> VMResult {
    args.check_args_num(0)?;
...
fn glob(vm: &mut VM, _self_val: Value, args: &Args2) -> VMResult {
    args.check_args_num(1)?;
